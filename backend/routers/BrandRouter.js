...
class BrandRouter {
  ...
  router() {
    ...
    // console.log("In the brand router");
    return router;
  }
  
  async get_all(req, res) {
    // console.log("reached brand backend");
    try {
      let getResult = await this.brandService.fetchAll();
  ...
  async get_id(req, res) {
    ...
    try {
      let brand = await this.brandService.fetchByID(id);
      return res.status(200).json({ message: "proceeded", brand });
    } catch (err) {
      if (err instanceof EvalError) {
  ...
  async post(req, res) {
    // console.log("reached brand backend");
    try {
      let regResult = await this.brandService.create(req.body);
  ...
  async put(req, res) {
    // console.log("reached brand backend");
    try {
      let putResult = await this.brandService.update(req.body);
  ...
  async delete_id(req, res) {
    ...
    if (Boolean(id) === false) { return res.status(406).json({ message: "id missing" }) };
    try {
      let deleteCount = await this.brandService.deleteByID(id);
